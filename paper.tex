...
\item[2.] \textit{Platform as aService (PaaS)}. Dem Nutzer werden zusätzlich Entwicklerwerkzeuge zur Verfügung gestellt, um Webanwendungen ohne lokale Installation entwickeln zu können. \textit{Microsoft Azure} ist ein Beispiel für diese Kategorie.
\item[3.] \textit{Software as aService (SaaS)}. Dem Nutzer wird die Verwendung einer Anwendung per Abonnement gestattet, sodass kostspielige Software zeitweise zu einem geringeren Preis genutzt werden kann. Ein Beispiel für diese Kategorie sind \textit{Microsoft 365} oder auch die \textit{Adobe Creative Cloud}.
\end{itemize}
...
Allerdings unterscheiden sich Fahrzeuge zu konventionellen Knoten insofern, dass sie nicht nur stehen, sondern sich auch bewegen können und daher über keinen festen Zugang zum Netzwerk verfügen.
...
Es wird davon ausgegangen, dass Fahrzeugbesitzer durch eine Entschädigung dazu angespornt werden könnten, die Ressourcen ihres Fahrzeugs zu Verfügung zu stellen.
...
Genauer betrachtet existierenden allerdings zusätzliche Herausforderungen, wie beispielsweise eine limitierte Stromversorgung.
Außerem können, wie in Kapitel \ref{sec:vc} beschrieben, auch dynamische \textit{Vehicular Clouds} existieren, in denen die Fahrzeuge sich bewegen.
...
Im Folgenden werden drei zusätzliche Cloud-Dienste vorgestellt, die durch \textit{Vehicular Clouds} realisierbar sind\cite{conti2013mobile}.
...
Es bietet sich für zuerst genannte Fahrzeuge an, ihre Internetverbindung den anderen Fahrzeugen zu Verfügung zu stellen.
Vergleicht man diesen Anwendungsfall mit der Nutzung von Smartphones, wird einem klar, dass in Zukunft vermutlich sämtliche Fahrzeuge über eine Internetverbindung per Mobilfunk oder WiFi verfügen.
Doch selbst dann bietet sich dieser Dienst an, da Fahrzeuge ihre überschüssige Bandbreite zu Verfügung stellen können, falls diese von anderen Fahrzeugen für beispielsweise Unterhaltungsanwendungen benötgt wird.
Es wird davon ausgegangen, dass die Bereitschaft die Bandbreite zu Verfügung zu stellen durch die jeweiligen Fahrzeuge verbreitet wird.
...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{images/networkAAS}
...
Das Fahrzeug \textbf{e} befindet sich in direkter Nähe zum \textbf{w} und kann somit diese Internetverbindung transient zu Verfügung stellen.
...
Fahrzeug-Ad-hoc-Netze wurden entwickelt, um den Verkehrsteilnehmern ein breites Spektrum an Diensten zu Verfügung zu stellen.
...
Für die Kommunikation zwischen den \textit{Clustern} wird \textit{Delay Tolerant Networking} genutzt, um nicht durch eine geringere Stabilität des Netzwerks aufgrund der neuen Herausforderungen von \textit{Vehicular Clouds} eingeschränkt zu werden.
Für die Kommunikation innerhalb von \textit{Clustern} auf \textit{content-based routing} gesetzt, sodass Nachrichten mittels Überprüfung des Inhalts an die Fahrzeuge weitergeleitet werden kann, die sich dafür interessieren, ohne zuvor explizit die Empfänger festzulegen.

\subsection{\textit{Storage as a Service (STaaS)}}
Eine weitere Idee besteht darin, dass Fahrzeuge auch ihren Speicherplatz zu Verfügung stellen könnten.
...
Je nach Anwendungsfall könnte allerdings selbst dann der Zugriff auf die gespeicherten Informationen per Internet erfolgen.
