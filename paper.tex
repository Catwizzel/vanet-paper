...
\label{sec:examples}
In diesem Kapitel werden Anwendungsbeispiele behandelt, um neue Möglichkeiten und Lösungen für bestehende Probleme mit \textit{Vehicular Clouds} vorzustellen.
...
Daraus ergibt sich, dass ungefähr 5000 Fahrzeuge ungenutzt am Flughafen stehen, während ihre Besitzer auf Reisen sind.
...
Selbiges gilt für Informationen, die gar nicht oder zu spät durch das eigene Fahrzeug erfasst werden können.
...
Generell kann der Austausch von Informationen innerhalb einer \textit{VC} den Verkehr auf unseren Straßen sicherer gestalten.
...
Auf der einen Seite müssen die Sicherheit und Privatsphäre des Fahrzeugs und Fahrzeugbesitzers gewahrt werden, welche die Ressourcen bereitstellen.
Andererseits müssen auch die Sicherheit und Privatsphäre der Nutzer gewahrt werden.
Dabei sind einige Herausforderungen und Lösungsmöglichkeiten aus anderen Netzen übertragbar.
...
Dadurch kann die Nachricht nur entschlüsselt werden, falls das Fahrzeug sich an der korrekten Position befindet.
...
Herkömmlicherweise versuchen Angreifer in das Netz einzudringen, allerdings sind alle Nutzer der \textit{Vehicular Cloud} ein Teil des Netzes, sodass sich eine Aussperrung der Angreifer schwierig gestaltet.
Im Folgenden werden Bedrohungen beschrieben, mit denen eine \textit{VC} umgehen können muss\cite{conti2013mobile}:
\begin{itemize}
\item \textit{Verschleierung der Identität}. Der Angreifer täuscht vor, ein anderer Nutzer zu sein, um Daten oder Vorteile zu erlangen, zu denen er nicht berechtigt ist.
...
\item \textit{Manipulation}. Der Angreifer manipuliert Daten, ändert Nachrichten oder verschickt eigene Nachrichten.
...
\item \textit{Denial of Service (DoS)}. Der Angreifer versucht durch eine große Menge von gezielten Anfragen die Ressourcen der \textit{VC} zu belegen und somit die Erreichbarkeit für andere Nutzer einzuschränken.
\item \textit{Rechteausweitung}. Der Angreifer nutzt eine Schwachstelle aus, um seine Zugriffsrechte innerhalb der \textit{VC} zu erhöhen.
...
\begin{itemize}
\item \textit{Integrität}. Nachrichten dürfen nicht modifiziert werden. Die Nachrichten müssen zuverlässig und korrekt sein.
\item \textit{Vertraulichkeit}. Unautorisierte Nutzer dürfen nicht auf sensible Nachrichten zugreifen.
\item \textit{Verfügbarkeit}. Nachrichten müssen verfügbar sein, wenn sie benötigt werden.
\item \textit{Authentifizierung}. Nachrichten müssen auf eine rechtmäßige Weise von autorisierten Knoten verschickt werden.
...
\item \textit{Echtzeitfähigekit}. Manche Anwendungen und Ereignisse, wie beispielsweise bei einem Unfall, sind zeitkritisch.
Daher muss die \textit{VC} fähig sein, zeitkritische Nachrichten übertragen zu können.
\item \textit{Privatsphäre}. Die Privatsphäre der Beteiligten muss gewahrt werden.
\item \textit{Sybil-Attacken-frei}. Nachrichten von nicht authentifizierten Nutzern dürfen nicht übertragen werden.
\end{itemize}
% ----------------------------------------------------------------------------------------------------------------------
...
Dabei wird auf die Terminologie von \textit{Dedicated Short-Range Communication (DSRC)} zurückgegriffen.
...
Zusätzlich wird auf die Terminologie von \textit{Cloud Computing} zurückgegriffen, und zwar die drei Begriffe \textit{Node-}, \textit{Cluster-} und \textit{Cloud Controller}.
...
Aktuelle Implementierungen von Virtualisierung bauen, wie in Abbildung \ref{fig:fullVirtual} gezeigt, auf der Idee auf, dass eine sogenannte \textit{Hypervisor}-Software die VMs kontrolliert und ihnen die Ressourcen zuweist.
Abbildung \ref{fig:obuVirtual} zeigt die notwendigen Anpassungen für eine Verwendung in \textit{VCs}.
...
Die Basisblöcke von CPU-Instruktionen würden durch den \textit{RS} an die \textit{OBUs} verteilt werden.
...
Die durchschnittliche Datenrate von DDR4-Arbeitsspeicher liegt zwischen 2 und 3Gbits und ist somit eine Mindestanforderung für die Funktion einer \textit{VC}.
...
Wenn ein Fahrzeug den \textit{Cluster} verlässt, muss der \textit{Cluster Controller} sicherstellen, dass weiterhin alle Anfragen verarbeitet werden.
...
Sollte sich in dem zuvor festgelegten Zeitraum kein neues Fahrzeug anmelden, werden benachbarte \textit{Cluster Controller} gefragt, ob sie einen Ersatz bereitstellen können.
...
Dabei entstehende Sicherheitsprobleme und Bedenken hinsichtlich der Privatsphäre wurden genannt und einige Lösungsansätze vorgestellt.
...
Da diese Ressourcen vermutlich nicht gänzlich ausgeschöpft werden, bietet sich eine Verwendung für \textit{Cloud Computing} an.
Für die eingebetteten Systeme der Fahrzeuge herrschen allerdings Limitierungen wie die Mobilität oder auch die Geschwindigkeit der Verbindung, sodass die klassischen Konzepte der Virtualisierung angepasst werden müssen.
